...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
  ...
  return {
    title: isTr ? "PIPPINU Rug Pull Analizi Vaka Çalışması | Taranoid" : "PIPPINU Rug Pull Analysis Case Study | Taranoid",
    description: isTr
      ? "Gerçek bir rug pull dolandırıcılığı nasıl gerçekleşti? PIPPINU projesinin blockchain analizini ve sahte tutucu tespitini adım adım inceleyin."
      : "How did a real rug pull scam happen? Step-by-step blockchain analysis and sybil wallet detection for the PIPPINU project.",
  };
}
...
export default async function PippinuCasePage({ params }: { params: Promise<{ lang: string }> }) {
  ...
  const articleSchema = {
    ...
    "@type": "Article",
    "headline": isTr ? "🔍 PIPPINU Rug Pull Analizi" : "🔍 PIPPINU Rug Pull Analysis",
