...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
    ...
    return {
        title: isTr ? "Hakkında — Taranoid Risk Analizi" : "About Us — Taranoid Rug Pull Checker",
        description: isTr
            ? "Taranoid nedir, nasıl çalışır? Solana blockchain analizi, akıllı kontrat güvenlik denetimi ve wash trading algılama platformumuz hakkında bilgi alın."
            : "What is Taranoid? Learn about our Solana blockchain analysis, smart contract security audits, and ML-powered wash trading detection platform.",
    };
}
                ...
                <div className="space-y-10">
                    ...
                    <div className="text-center md:text-left mb-12">
                        ...
                        <p className="text-lg md:text-xl leading-relaxed max-w-3xl" style={{ color: "var(--cg-text-muted)" }}>
                            ...
                            yatırımcılara kurumsal düzeyde blockchain güvenlik denetimi (smart contract audit) sunan
                            bir risk değerlendirme platformudur. Makine öğrenimi algoritmalarıyla donatılmış 9 metrikli motoru, <span className="font-black" style={{ color: "var(--cg-text)" }}>AI destekli Türkçe rapor</span> ve{" "}
                            <span className="font-black" style={{ color: "var(--cg-text)" }}>creator profiling</span> ile işlem ağacı (transaction graph) haritalandırması yaparak kullanıcıları{" "}
                            <span className="font-black underline decoration-red-500/30 underline-offset-4" style={{ color: "var(--cg-text)" }}>
                                sahte projeler (honeypot), rug pull'lar ve cüzdan kümeleme manipülasyonlarından
                            </span>{" "}
                            koruruz.
                        </p>
                    </div>
